...
This is a small example file of how to use the fits module of astropy in application to matisse raw data
...
dir="D:\\Documents\\Travail\\MATISSE\\testplan\\aquaNoise\\"
filename=dir+"10_0T.fits"
...
primary=hdulist[0]                          # The first table is normally the primary table which contains the main header but no table in the case of ESO data                  
...
#---------------------------------------------------------------------------------
imdet=hdulist[1]                            # The second table is usually the imaging_detector table listing the detector regions recroded in the file. 
...
cols[0]                                     # Return the name and format of the first column of the data => 'REGION' and 'I' for int
imdet.data                                  # the data in the table          
...
interfsize=imdet.data[10].field('NAXIS')    # return the [x,y] size of the 11th region of the detector (usually the interferometric one)
interfcorner=imdet.data[10].field('CORNER') # return the upper left corner position in absolute coordinate of all 11th detector region.
regnames=imdet.data.field('REGNAME')        # return the names of the all the regions of the detectors 'CAL1' ... 'PHOT1_1' 'INTERF_1' ....
#---------------------------------------------------------------------------------                                              
imdata=hdulist[2]                           # The third table is usually the imaging_data table. You can check before in the hdunames list.
...
datainterf=imdata.data.field('DATA11')      # Return the data cube of corresponding to the 11th region of the detector (usually the interferometric one)
