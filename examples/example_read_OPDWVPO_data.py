...
filename = dir+"OI_OPDWVPO_CORRFLUX_CAL_MATISSE_GEN_LAMP_L108_0001.fits.gz"
...
#---------------------------------------------------------------------------------
imdet        = hdulist['OI_OPD']   # The second table is usually the imaging_detector table listing the detector regions recroded in the file. 
...
print nrow
opd     = imdet.data['OPD']
time     = imdet.data['TIME']

opd2    = np.reshape(opd,[opd.shape[0]/6,6])
time2    = np.reshape(time,[time.shape[0]/6,6])

plt.plot(time2-np.min(time2),opd2)