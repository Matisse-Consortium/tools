...
echo "------------1-------------"
if($algoMode == 1) then
  set resultdir = BIS.$objname.F$fov.pix$npix.Cost$costFunc.weightPow$weightPower.qrec$qrecmode.mu$muStarts[1].reg$regFuncs[1].wiener$wienerfilter.engine$engine.Script1.E.1
  set name = BIS.$objname.F$fov.pix$npix.weightPow$weightPower.qrec$qrecmode.mu$muStarts[1].reg$regFuncs[1].wiener$wienerfilter.engine$engine
endif
if($algoMode == 2) then
  set resultdir = FT.$objname.F$fov.pix$npix.Cost$costFunc.weightPow$weightPower.qrec$qrecmode.mu$muStarts[1].reg$regFuncs[1].wiener$wienerfilter.engine$engine.Script1.E.1
  set name = FT.$objname.F$fov.pix$npix.weightPow$weightPower.qrec$qrecmode.mu$muStarts[1].reg$regFuncs[1].wiener$wienerfilter.engine$engine
endif
if($algoMode == 3) then
  set resultdir = FTBIS.$objname.F$fov.pix$npix.Cost$costFunc.weightPow$weightPower.qrec$qrecmode.mu$muStarts[1].reg$regFuncs[1].wiener$wienerfilter.engine$engine.Script1.E.1
  set name = FTBIS.$objname.F$fov.pix$npix.weightPow$weightPower.qrec$qrecmode.mu$muStarts[1].reg$regFuncs[1].wiener$wienerfilter.engine$engine
endif
set i = 1
label1:
if(-d $resultdir) then
...
#       qrec built from the phase measurements only ($qrecmode == 2)
   ...
   if($algoMode == 1) then
     rm -f head03; echo "#  " >> head03; echo "# 2) reconstructions sorted with increasing CP-qrec (CP-qrec in the last column; qrecmode=2):" >> head03
   else
     rm -f head03; echo "#  " >> head03; echo "# 2) reconstructions sorted with increasing PH-qrec (PH-qrec in the last column; qrecmode=2):" >> head03
   endif
   rm -f head04; echo "#  " >> head04; echo "# 1) reconstructions sorted with increasing qrec (qrecmode=1):" >> head04
...
#  set fits = `echo rec_*.fits`; ls -l $fits 
   rm -f bestrec.qrec1.fits; cp $dir0qrec/rec_*.fits   bestrec.qrec1.fits
   rm -f bestrec.qrec2.fits; cp $dir0cpqrec/rec_*.fits bestrec.qrec2.fits
...
echo "------------------------------------------------------------------------------------------------------------------------------------------------"
# if($qrecmode == 2) echo "---> the reconstructions of all runs are displayed sorted with according decreasing quality (using phase qrec (qrecmode=2))"
# if($qrecmode == 1) echo "---> the reconstructions of all runs are displayed sorted with according decreasing quality (using phase+visibility qrec (qrecmode=1))"
...
echo "------------------------------------------------------------------------------------------------------------------------------------------------"
echo "---> best reconstruction according to qrec (qrecmode=1) : bestrec.qrec1.fits "
ls -l $resultdir/bestrec.qrec1.fits
echo "---> best reconstruction according to qrec (qrecmode=2) : bestrec.qrec2.fits " 
