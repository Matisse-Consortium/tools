...
def plot_config(xlabel, ylabel, title, ax, dic, xlim=(None, None), ylim=(None, None),plot_legend=True,
legend_loc='upper right',annotate=True,annotate_va='top',annotate_fontsize=8,annotate_xy=(0,1)):
    ...
    if plot_legend == True:
        ax.legend(loc=legend_loc, fontsize=8, fancybox=True, framealpha=0.5)
    if annotate == True:
        ax.set_title(title)
        try:
...
def open_fits(fits_file, verbose=False):
    dic = {}
    try:
        ...
        try:
            ...
            dic['VIS']['VISPHI'] = hdu['OI_VIS'].data['VISPHI']
            dic['VIS']['VISPHIERR'] = hdu['OI_VIS'].data['VISPHIERR']
            # try:
            #     dic['VIS']['CFXAMP'] = hdu['OI_VIS'].data['CFXAMP']
            #     dic['VIS']['CFXAMPERR'] = hdu['OI_VIS'].data['CFXAMPERR']
            # except KeyError as e:
            #     pass
            #     if verbose:
            ...
            dic['VIS']['MJD'] = hdu['OI_VIS'].data['MJD']
            ...
            if verbose:
                ...
                print(e)
        try:
            dic['VIS']['AMPTYP'] = hdu['OI_VIS'].header['AMPTYP']
        except KeyError as e:
            if verbose:
        ...
        try:
            ...
            dic['VIS2']['MJD'] = hdu['OI_VIS2'].data['MJD']
        ...
        try:
            ...
            dic['TF2']['MJD'] = hdu['TF2'].data['MJD']
            dic['TF2']['STA_INDEX'] = hdu['TF2'].data['STA_INDEX']
            dic['TF2']['FLAG'] = hdu['TF2'].data['FLAG']
        except KeyError as e:
            dic['TF2'] = {}
            dic['TF2']['TF2'] = []
            dic['TF2']['TF2ERR'] = []
        ...
        try:
            ...
            dic['T3']['T3PHI'] = hdu['OI_T3'].data['T3PHI']
            dic['T3']['T3PHIERR'] = hdu['OI_T3'].data['T3PHIERR']
        ...
        try:
            ...
            dic['FLUX']['MJD'] = hdu['OI_FLUX'].data['MJD']
...
def make_uv_tracks(uv, inp, flag, ax, bases=[], symbol='x'):
    #
    ...
    if base not in bases:
        ...
        ax.plot(ul, vl, '-', color='0')
        ax.plot(-ul, -vl, '-', color='0')
        ax.plot([0.], [0.], '+k', markersize=5, markeredgewidth=2)
        ax.text(-u-2, -v-2, base, color='0',alpha=0.6)
        bases.append(base)
...
def make_uv_plot(dic,ax,verbose=False,annotate=True,B_lim=(np.nan,np.nan),figsize=(5,5)):
    ...
    try:
        ...
        if math.isnan(B_lim[0]):
            ...
            xlim = (+B_lim[1],-B_lim[1])
            ylim = (-B_lim[1],+B_lim[1])
...
def make_plot_with_baseline(dic,ax,tag,oi_type='vis',ylabel='',verbose=False,annotate=True,annotate_baselines=False,B_lim=(np.nan,np.nan),
    ...
    try:
        ...
        if not np.all(np.isnan(y_new)):
            ...
            sc_plot = ax.scatter(pbl, y_new,c=pbla,vmin=0.0,vmax=180.0,marker='o',edgecolor='black',zorder=5,label='Data') #'_nolegend_')
            if fit_model == True:
...
def make_plot_with_wavelength(dic,ax,tag,oi_type='vis',ylabel='',verbose=False,annotate=True,wl_lim=(np.nan,np.nan),
    figsize=(5,5),input_fitsfile='',legend_loc='upper right'):
    ...
    try:
        ...
        for j in for_range:
            ...
            if oi_type == 'flux':
                ...
                pmin = np.nanpercentile(y_new[int(0.2*Ny):int(0.8*Ny)], 5.0)
                pmax = np.nanpercentile(y_new[int(0.2*Ny):int(0.8*Ny)], 95.0)
                pmid = (pmin + pmax) / 2.0
                pdiff = pmax - pmin
                lim_fact = 1.5
        ...
        plot_config(xlabel, ylabel,plot_title, ax, dic,
                    ylim=(np.nanmin(ymin), np.nanmax(ymax)),xlim=wl_lim_temp,annotate=annotate,legend_loc=legend_loc)
...
B_lim=(np.nan,np.nan),fit_model = False,figsize=(15,15)):
    # check if output directory exists
    # if not, create it
    fitsfiles = sorted(glob.glob(datadir + '/*' + fn_pattern + '*fits'))
    if fitsfiles != []:
...
wl_lim=(np.nan,np.nan),B_lim=(np.nan,np.nan),fit_model = False,figsize=(5,5)):
    ...
    if dic:
        ...
        if 'RAW_DPHASE' in input_fitsfile:
            oi_types = ['visamp','visphi']
            ylabels = ['VISAMP', 'VISPHI ($^\circ$)'] 
...
def make_mosaic_plot(input_fitsfile_list,ouput_file_path,oi_types_list=[['uv','vis_pbl','visamp_pbl','flux','vis','visamp','visphi','t3phi']], 
verbose=False, save_png=True, save_eps=False,sel_wl=np.nan,bandwidth=np.nan,annotate=True,legend_loc='upper right',
legend_loc_pbl='lower left',wl_lim=(np.nan,np.nan),B_lim=(np.nan,np.nan),fit_model = False,figsize=(15,15)):
            ...
            for oi_type in oi_types_list[i]:
                if oi_type == 'uv':
                    ...
                    if oi_type == 'vis_pbl' or oi_type == 'visamp_pbl':
                        if oi_type == 'vis_pbl': 
                            if ('CAL_INT' not in input_fitsfile): ylabel = 'Raw visibility'
                        ...
                        if oi_type == 'flux':
                            if ('CAL_INT' not in input_fitsfile): ylabel = 'Raw total flux'
                        ...
                        if oi_type == 'visamp':
                            if 'correlated' in dic['VIS']['AMPTYP']:
                                ...
                                tag = 'visamp'
                        if oi_type == 'visphi':
                            if ('CAL_INT' not in input_fitsfile): ylabel = 'Raw differential phase ($^\circ$)'
                            else: ylabel = 'Differential phase ($^\circ$)'
                            tag = 'visphi'
                        if oi_type == 't3phi':
                            if ('CAL_INT' not in input_fitsfile): ylabel = 'Raw closure phase ($^\circ$)'
                            else: ylabel = 'Closure phase ($^\circ$)'
                            tag = 't3phi'
                        if oi_type == 't3amp':
                            if ('CAL_INT' not in input_fitsfile): ylabel = 'Raw T3AMP'
                            else: ylabel = 'T3AMP'
                            tag = 't3amp'
    ...
    if save_png: plt.savefig(ouput_file_path + '.png', dpi=200)
    if save_eps: plt.savefig(ouput_file_path + '.eps', format='eps', dpi=300)
    plt.close(outfig)
    plt.close('all')
...
verbose=False, save_png=True, save_eps=False,sel_wl=3.6,bandwidth=0.2,file_type='',pro_catg='',
annotate=False,wl_lim=(np.nan,np.nan),B_lim=(np.nan,np.nan)):
    # check if output directory exists
    # if not, create it
    if input_fitsfiles != []:
        if not os.path.exists(outputdir):
